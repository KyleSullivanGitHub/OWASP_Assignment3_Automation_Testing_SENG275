...
public class LoginMk2 implements ITest
{
    ...
    public void LG1_Valid_Input(String chosenBrowser) throws IOException, InterruptedException
    {
     ...
     */
    @Test(
            groups = {"Smoke","Login Smoke","Login","noDataProvider"},
            priority = 1,
            enabled = true
    )
    public void LG2_Invalid_Input() throws InterruptedException, IOException
    {
        ...
        try {
            ...
            TestFunctions.manualLogin(browserWindow,"pswrd");
    ...
    public void LG3_Valid_Input(String chosenBrowser, Object[] dataSet) throws InterruptedException, IOException {
        ...
        try {
             ...
             */
            fillOutLogGoogle(browserWindow, dataSet);
...
*/
    ...
    private void fillOutLogGoogle(WebDriver browserWindow, Object[] dataset) throws InterruptedException
    {
        TestFunctions.navToLogin(browserWindow);

        TestFunctions.waitForSite(browserWindow,TestFunctions.loginButtonG,true);
        Thread.sleep(500);

        if(!browserWindow.getCurrentUrl().startsWith(TestFunctions.website))
        {
            TestFunctions.waitForSite(browserWindow, TestFunctions.identifierID);
            try
            {
                WebElement emailUser = browserWindow.findElement(By.cssSelector(TestFunctions.identifierID));
                emailUser.click();
                emailUser.sendKeys(""+dataset[0] + Keys.ENTER);//enter email
            }
            catch (NoSuchElementException invalidCase)
            {
                assertTrue(dataset[0] != TestFunctions.googleEmail);
                return;
            }
            TestFunctions.waitForSite(browserWindow, TestFunctions.googlePasswordTarget);
            try
            {
                WebElement passwordInput = browserWindow.findElement(By.cssSelector(TestFunctions.googlePasswordTarget));
                Thread.sleep(500);
                passwordInput.click();
                passwordInput.sendKeys(""+dataset[1] + Keys.ENTER);//enter password
            }
            catch (NoSuchElementException invalidCase)
            {
                assertTrue(dataset[1] != TestFunctions.googlePassword);
                return;
            }
        }
        while(!browserWindow.getCurrentUrl().equals(TestFunctions.website))
            Thread.sleep(100);
    }
